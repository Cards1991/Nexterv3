...
async function popularSelectFuncionariosFalta() {
    ...
    fsnap.forEach(d => { 
        const f = d.data(); 
        if (f.condicao === 'Férias' || f.condicao === 'Trabalho Externo' || (f.condicao && f.condicao.startsWith('Afastado'))) return; // Ignora férias e afastados e externos
        const o = document.createElement('option'); 
        o.value = d.id; 
...
function visualizarFaltasPorSetor(setor) {
    ...
    faltasDoSetor.forEach(f => {
        const id = f.funcionarioId || f.funcionarioNome; // Fallback to name if ID is missing
        if (!faltasPorColaborador[id]) {
            faltasPorColaborador[id] = { id: id, nome: f.funcionarioNome, total: 0 };
        }
        faltasPorColaborador[id].total += 1;
    });

    const ranking = Object.values(faltasPorColaborador).sort((a, b) => b.total - a.total);

    let html = `
        <h6 class="fw-bold mt-3 mb-3 text-primary d-flex align-items-center">
            <i class="fas fa-trophy fs-4 text-warning me-2" style="filter: drop-shadow(0 2px 4px rgba(255,193,7,.4));"></i>
            Ranking de Faltas - ${setor}
        </h6>
        
        <div class="row gx-3 mb-4">
            ${ranking.map((r, i) => {
                let badgeClass = 'bg-secondary';
                let iconClass = 'fa-medal text-white';
                let cardStyle = '';
                
                if (i === 0) { 
                    badgeClass = 'bg-danger bg-gradient';
                    iconClass = 'fa-crown text-warning';
                    cardStyle = 'border: 2px solid #dc3545; transform: scale(1.02); z-index: 2; box-shadow: 0 .5rem 1rem rgba(220,53,69,.15)!important;';
                } else if (i === 1) { 
                    badgeClass = 'bg-warning text-dark bg-gradient';
                    iconClass = 'fa-medal text-dark';
                } else if (i === 2) { 
                    badgeClass = 'bg-info text-dark bg-gradient';
                }

                return `
                <div class="col-md-4 mb-2">
                    <div class="card shadow-sm h-100" style="border-radius: 12px; transition: all 0.2s; ${cardStyle}">
                        <div class="card-body p-3 d-flex flex-column justify-content-between text-center position-relative">
                            <span class="badge ${badgeClass} position-absolute top-0 start-50 translate-middle rounded-pill px-3 py-2 shadow" style="font-size: 0.9rem;">
                                <i class="fas ${iconClass} me-1"></i> ${i + 1}º Lugar
                            </span>
                            <div class="mt-3 mb-2">
                                <h6 class="fw-bold text-dark mb-0 text-truncate" title="${r.nome}">${r.nome}</h6>
                            </div>
                            <div class="d-flex justify-content-center align-items-center gap-2 mt-auto">
                                <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 rounded-pill px-3 py-2" style="font-size: 0.85rem;">
                                    <i class="fas fa-calendar-times me-1"></i> ${r.total} falta(s)
                                </span>
                                <button class="btn btn-sm btn-primary rounded-circle shadow-sm" style="width: 32px; height: 32px; padding: 0;" onclick="chamarHistoricoColaborador('${r.id}')" title="Explodir Histórico">
        ...
        <h6 class="fw-bold mb-3 d-flex align-items-center">
            <i class="fas fa-list-ul text-secondary me-2"></i> Detalhamento Completo
        </h6>
        <div class="table-responsive">
                ...
                <tbody>
                    ${faltasDoSetor.map(f => `
                        <tr>
                            ...
                            <td>${f.periodo}</td>
                            <td>
                                <button class="btn btn-sm btn-outline-primary" onclick="editarFalta('${f.id}')" title="Editar Falta">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-secondary" onclick="chamarHistoricoColaborador('${f.funcionarioId || f.funcionarioNome}')" title="Ver Histórico">
...
window.chamarHistoricoColaborador = async function(id) {
    if (typeof window.showCollaboratorHistoryModal === 'function') {
        window.showCollaboratorHistoryModal(id);
    } else {
        console.error("Função showCollaboratorHistoryModal não encontrada!");
    }
};
